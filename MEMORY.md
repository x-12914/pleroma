...
- ✅ UI scan-aggregation (2026-06-23, deployed): `GET /logs/sources` groups NETWORK_TRAFFIC detections by src_ip (count, distinct classes, distinct ports, worst verdict, time span) in Postgres; Logs page gains a Timeline / By-source toggle. Collapses ~340k raw rows into ~500 source campaigns (noisiest first) — the dashboard-readability fix for the scan flood. Verified live. Frontend deploy: built locally (`npm run build`) + tar-piped to `/opt/pleroma/frontend-dist` with atomic swap; nginx serves new bundle.
